...
const wrapper = await readFile(new URL('src/observatory-index.js', root), 'utf8');
...
for (const token of ['tools/list', 'tools/call', '/api/observatory', 'writeReceipt', 'receipt-backed-observability']) {
  if (!wrapper.includes(token) && !observatory.includes(token)) throw new Error(`observatory wrapper missing ${token}`);
}
for (const name of [
  'route_front_desk',
...
for (const token of ['"binding": "DB"', '"binding": "ASSETS"', '"/mcp"', '"main": "src/observatory-index.js"']) {
...
console.log(`Deployment validation passed: ${required.length} required files, 10 read-only MCP tools, 4 observatory views, 3 D1 tables.`);
