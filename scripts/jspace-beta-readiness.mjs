...
const report = {
  schema: 'agentropolis.jspace.beta-readiness.v2',
...
report.gates.PROD_OPS_GREEN = await required('src/jspace-projection.js', ['activateProjectionRevision','listProjectionRevisions','projectionFreshness','STALE_UNKNOWN_AGE']) &&
  await required('src/jspace-index.js', ['/api/jspace/projection/history','/api/jspace/projection/activate','ALLOW_DERIVED_CACHE_POINTER_WRITE','STALE_DERIVED_PROJECTION']);
...
const sourceReady = ['SOURCE_GREEN','UI_BETA_GREEN','PROD_OPS_GREEN','DEPLOY_CORRIDOR_PRESENT','RUNBOOK_PRESENT'].every((k)=>report.gates[k]);
report.releaseClass = sourceReady ? 'SOURCE_UI_AND_PROD_OPS_BETA_READY' : 'SOURCE_NOT_READY';
